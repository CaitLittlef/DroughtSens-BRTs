...
for.mod <- read.csv(paste0(outdir,"/S_mod_forest_n10000_LR0.01_TC5_LO.PRED_noEVInoC_2018-10-28.csv")) %>% dplyr::select(-X)
...
list <- list(for.mod, for.sev, stp.mod, stp.sev)

## Create folder for plots
currentDate <- Sys.Date()
dir.create(paste0(outdir, "/AllDroughts_noEVInoC_", currentDate))
plotdir <- paste0(outdir, "/AllDroughts_noEVInoC_", currentDate)
...
for(i in 1:length(explan.vars)){
  ...
  temp.lo <- temp.lo %>% filter(var == explan.vars[i]) %>% arrange(x) 
  temp.lo.lo <- loess(temp.lo$y ~ temp.lo$x, span = 0.5) 
  plot(temp.lo.lo$x, temp.lo.lo$fitted, col = "brown", lty=1, type='l',
       lwd=2, ylab="Smoothed fitted functions", xlab=explan.vars.names[i], ylim=c(ymin,ymax),xlim=c(xmin,xmax),
