...
var.mat <- matrix(unlist(var), ncol = 8, byrow = TRUE) 
rel.inf.mat <- matrix(unlist(as.numeric(rel.inf)), ncol = 8, byrow = TRUE) 
...
csvFileName <- paste0(drought.type,"_stats_noEVInoC_",currentDate,".csv")
...
for (i in rows){
  ...
  col.to <- unname(unlist(stats.row[,5:12])) %>% as.character()
  
  # Define which columns I want to change: they'll go from FROM to TO.
  col.from <- colnames(stats.row[,13:20]) 
  ...
  stats.row <- stats.row %>% dplyr::select(-(5:12)) 
...
# Write to new csv
currentDate <- Sys.Date()
write.csv(stats.new, paste0(outdir,"/",drought.type,"_stats_ORDERED_noEVInoC_",currentDate,".csv"))
...
# Write to new csv
currentDate <- Sys.Date()
write.csv(stats.sum, paste0(outdir,"/",drought.type,"_stats_SUMMARY_noEVInoC_",currentDate,".csv"))
...
explan.vars
explan.vars.names <- c("Drought exposure (%)",
...
currentDate <- Sys.Date()
dir.create(paste0(outdir, "/", drought.type, "_noEVInoC_", currentDate))
plotdir <- paste0(outdir, "/", drought.type, "_noEVInoC_", currentDate)
...
currentDate <- Sys.Date()
write.csv(all.lo, paste0(outdir, "/", drought.type, "_LO.PRED_noEVInoC_", currentDate,".csv"))
